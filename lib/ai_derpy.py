...
class AiDerpy(Player):
    def __init__(self):
        super().__init__()

    def start_game(self, my_symbol):
        super().start_game(my_symbol)
        self.filename = "ai_derpy.h5"
        self.model = self.init_model()
        self.states = []

    def end_game(self, is_winner, board):
        for state in self.states:
            self.save_result_for_player(state, self.my_symbol, is_winner)
            self.save_result_for_player(state, other_symbol(self.my_symbol),
                                    not is_winner)
        keras.backend.clear_session()

    def init_model(self):
        model = Sequential()
        model.add(Dense(1000, activation='relu', input_shape=(15, 15, 2)))
        model.add(Flatten())
